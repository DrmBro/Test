	...
	$.fn.xMenu = function(options) {
		return $(this).each(function() {
			...
			var $li =$selectUl.find("li");
				...
				var sf=document.reg_form.user_sf.value;
				
				 
				name = name.substring(0,name.length-1);
				id = id.substring(0,id.length-1);
				if(owl.hiddenID=="selectposhidden") {	
				   	$.post('check_km.action',{'v':name,'sf':sf},function(data){	
						if(data!="") {
							 //alert(data);
							 $("#ksfb").html(data);
						}
				   ...
				   ch = new Array;
			       ch = name.split(",");
				   document.reg_form.aabb.length = 0;
				   document.reg_form.aabb.options[0] = new Option('==请选择==','');
				   var dd=1;	
				   for(i=0;i<ch.length;i++){
					   document.reg_form.aabb.options[dd] = new Option(ch[i],ch[i]);
					   dd++;
				   }
